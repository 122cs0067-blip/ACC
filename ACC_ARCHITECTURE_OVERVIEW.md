...
graph TB
    ...
    subgraph Core ["ACC Core: Safety Monitor"]
        direction TB
        C -. "Latent Path" .-> E["i-ppDRE Density Sensor<br/>(RFF Projection)"]
...
def active_conformal_control(prompt, student, teacher, manifold, lambda_star):
    ...
    for t in range(max_tokens):
        ...
        # 2. Density Chasm Detection (i-ppDRE)
...
2.  **Logic Sensor (i-ppDRE)**: A non-parametric density ratio estimator that detects when the student enters a "Density Chasm"—a region of the latent space where the teacher model (AMX-accelerated) would behave differently.
